...
log noop log_ ( model, cmd ) =
    let
        _ =
            Debug.log "Log: " log_

        model_ =
            { model | logs = log_ :: model.logs |> List.take 2000 }

        extraCmd =
            sendSlackMessage Env.slackApiToken Env.slackChannel log_
                |> Task.attempt (\_ -> noop)
    in
    ( model_, Cmd.batch [ cmd, extraCmd ] )


rpcLog noop log_ ( a_, model, cmds ) =
    let
        ( newModel, newCmd ) =
            log noop log_ ( model, cmds )
    in
    ( a_, newModel, newCmd )
...
{-
   GraphQL error casting
-}


resultToSimpleErrorResult : Result (Graphql.Http.Error parsedData) parsedData -> Result String parsedData
resultToSimpleErrorResult result =
    case result of
        Ok res ->
            Ok res
...
errorToString error =
    case error of
        Graphql.Http.GraphqlError possiblyParsedData graphqlErrors ->
            "GraphQL Error: " ++ String.join ", " (List.map .message graphqlErrors)
        ...
        Graphql.Http.HttpError httpError ->
            case httpError of
                ...
                Graphql.Http.BadPayload jsonError ->
                    "Bad payload: " ++ Decode.errorToString jsonError
